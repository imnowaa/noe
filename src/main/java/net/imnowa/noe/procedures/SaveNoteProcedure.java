	...
	public static void execute(IWorld world, double x, double y, double z, Entity entity, HashMap guistate) {
		...
		if (world instanceof World) {
			if (!((World) world).isRemote()) {
				((World) world).playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("noe:typing")), SoundCategory.NEUTRAL, (float) 0.5, 1);
			} else {
				((World) world).playSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("noe:typing")), SoundCategory.NEUTRAL, (float) 0.5, 1, false);
			}
		}
